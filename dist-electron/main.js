...
function getStore() {
  ...
  return { scanHistory: [], license: { tier: "free", email: "" } };
}
...
class SecurityScanner {
  constructor() {
    this.testedUrls = /* @__PURE__ */ new Set();
    this.testedParams = /* @__PURE__ */ new Set();
  }
  ...
  async scanSQLi(url) {
    ...
    try {
      ...
      const urlsToTest = [.../* @__PURE__ */ new Set([url, ...links, ...apiEndpoints])];
...
function parseSSLCertificate(cert, hostname) {
  ...
  const daysRemaining = Math.floor((validTo - now) / (1e3 * 60 * 60 * 24));
...
function checkDomainMatch(hostname, cn, san) {
  ...
  if (san) {
    return san.split(", ").some(
      (alt) => checkPattern(alt.replace("DNS:", ""), hostname)
    );
  }
  return false;
}
...
async function scanNetwork(hostname) {
  try {
    ...
    const scanPromises = TOP_CRITICAL_PORTS.map((portObj) => {
      return checkPort(hostname, portObj.port, 2e3).then((status) => {
        if (status === "open") {
...
function checkPort(hostname, port, timeout = 2e3) {
  return new Promise((resolve) => {
...
ipcMain.handle("scan-local-network", async (event) => {
  try {
    ...
    return { success: true, data: { ...ports, timestamp: (/* @__PURE__ */ new Date()).toISOString() } };
  } catch (error) {
    return { success: false, error: error.message };
  }
});
...
ipcMain.handle("generate-pdf-report", async (event, scanData) => {
  try {
    const fileName = `GRIME_Security_Report_${Date.now()}.pdf`;
